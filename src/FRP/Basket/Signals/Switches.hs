...
hSwitch sf handler = Signal $ \t totalState a -> 
...
regulator sf regulator = 
  Signal $ \t s a -> let (b, s') = runSignal sf t s a 
                     in runSignal regulator t s (b, s')


parSwitch :: Signal s a (Either b c) -> Signal s b d -> Signal s c d -> Signal s a d
parSwitch main left right = 
